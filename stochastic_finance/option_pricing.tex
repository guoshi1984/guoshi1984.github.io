...
\noindent{\bf 2. Number of jumps during time interval $dt$: Possion process}\\
Now we consider the number of jumps in a given time interval $dt$, and we call it $N(dt)$. The average jump during $dt$ is $\lambda dt$. So we can get the probability distribution by substitude $\lambda dt$ to $\lambda$
\begin{align*}
	P(N(dt) = k) = \frac{(\lambda dt)^k}{k!}e^{-\lambda dt}
\end{align*}
The mean of $N(dt)$ is $\lambda dt$.\\
...
\begin{align*}
	...
	\frac{S(t_i) - S(t_{i-})}{S(t_{i-})} =  y - 1
	 \delta S(t) =  (y - 1) S(t) \delta N(t)  
\end{align*}
Where $dN(t) = 1$ when jump occurs, $0$ otherwise. So when $N(t) = 1$, in other words, if there is one jump from time zero up to t, $S(t) = yS(0)$. When $N(t) = 2$, $S(t) = y^2 S(0)$. So for any abitrary $N(t)$
\begin{align*}
	S(t) = S(0)y^{N(t)} 
\end{align*}
In order to fulfill the martingale condition, we write
\begin{align*}
	dS(t) = (y - 1)S(t) dN(t) - y\lambda S(t) dt
\end{align*}
Its integral form is
\begin{align*}
	S(t) = S(0)y^{N(t)} exp(-y\lambda t) 
\end{align*}
