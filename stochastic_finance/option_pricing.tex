...
\begin{align}
	\phi(u) = exp(C(\tau, u) + D(\tau, u)V_0 + iux_t) \label{heston_characteristic}
\end{align}
...
\begin{align*}
	...
	+ \sqrt{1 - \rho^2} \hat W_2) 
\end{align*}
All these above belong to the Euler discretization schemes which lead to more or less discretization errors. There exists an exact but computationaly expensive simulation scheme proposed by Broadie and Kaya\ref{exact_heston} by sampling the transition probability of $V(t)$ given $V(u)$（$u < t$) using chi-square distribution.
3. Bates process\\
\begin{align*}
