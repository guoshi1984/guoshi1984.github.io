...
If $f(t)= f(t+T)$ then $f(t)$ can be written as
\begin{align*}
f(t) = \sum_{-\infty}^{+\infty} c_k e^{\frac{2 \pi i k t}{T}}	
\end{align*}
...
{\bf Example}
\begin{align*}
f(t) = cos(2 \pi f t) = \frac{1}{2} (e^{i 2 \pi f t} + e^{-i 2 \pi f t})
\end{align*}
\begin{align*}
	c_k & = \frac{1}{T} \int_0^T f(t) e^{-i\frac{2 \pi k t}{T}} dt \\
	    & = \frac{1}{T} \int_0^T \frac{1}{2} (e^{i 2 \pi f t} + e^{-i 2 \pi f t}) e^{-i\frac{2 \pi k t}{T}} dt 
\end{align*}
...
\end{align*}
\begin{align*}
	F(f) = e^{-2 \pi^2 \sigma^2 f^2}
\end{align*}
So the Fourier transform of a Gaussian function is another Gaussian function.\\
