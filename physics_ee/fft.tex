...
\noindent 4) $\mathcal F(k) = \mathcal F(k+N)$. We remember $\Delta f = \frac{1}{T}$, so the Fourier transform function is also periodic with period $\frac{N}{T}$. Therefore, it is sufficient enough for us to confine k to be within the range $-N/2 < k \leq N$. For all the integers of $k^'$ which are beyond this range, we can an equivalent integer of k within $-N/2 < k \leq N$ that satisfies $\mathcal F(k^{'}) = \mathcal F(k)$.  With sample frequency $F_s = \frac{N}{T}$, the maximum frequecy of the signal(bandwidth B) we can tell is $B = \frac{N}{2T} = \frac{F_s}{2}$. In other words, in order to capture the whole bandwidth B of the signal, we must have $F_s \leq 2B$. This is Nyquist sampling theorem.\\
