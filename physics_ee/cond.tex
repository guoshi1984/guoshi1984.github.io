...
The solutions to the Schrödinger equation in a periodic potential take the form:
\begin{align}
\psi_{n\kvec}(\rvec) = e^{i\kvec\cdot\rvec} u_{n\kvec}(\rvec)
\end{align}
where $u_{n\kvec}(\rvec + \mathbf{R}) = u_{n\kvec}(\rvec)$ has the lattice periodicity.

\subsubsection{Energy Gap Formation}
At the Brillouin zone boundaries, where the Bragg condition $2\kvec \cdot \mathbf{G} = G^2$ is satisfied (with $\mathbf{G}$ a reciprocal lattice vector), degenerate free electron states mix strongly. For a 1D crystal with lattice constant $a$:
\begin{align}
\text{At } k = \pm \frac{\pi}{a}: \quad E_{\pm} = \frac{\hbar^2}{2m}\left(\frac{\pi}{a}\right)^2 \pm |V_G|
\end{align}
where $V_G$ is the Fourier component of the potential. This opens an \textbf{energy gap} of size $2|V_G|$.

\subsubsection{Dispersion Relation Near Zone Boundary}
For $k$ near $\pi/a$:
\begin{align}
E_{\pm}(k) = \frac{\hbar^2}{2m}\left[\left(\frac{\pi}{a}\right)^2 + q^2\right] \pm \sqrt{\left(\frac{\hbar^2\pi q}{ma}\right)^2 + |V_G|^2}
\end{align}
where $q = k - \pi/a$. The group velocity $v_g = \frac{1}{\hbar}\frac{dE}{dk}$ vanishes at the zone boundary.
