...
\noindent 1) We review the standard conductivity theory at classical level.\\
\noindent 2) We then move to the semiclassical level where the conductivity is related to the group velocity of the wavepacket and the Hall conductivity is derived. \\
\noindent 3) Finally, we go to the purely quantum level. We derive current by calculating the expectation value of  the current operator. The expectation has two components: the first term, the gradient of the energy eigenvalue in k space, contributes the conductivity of most metals; the second term, which relates to the Berry curvature, is nontrivial for the systems which break time-reversal symmetry.\\
\noindent 3) We show that the Berry curvature related conductivity term has an interesting property. The integral of the Berry curvature over the first Brillouin zone summed over all bands is an integer multiple of $2\pi$.\\
