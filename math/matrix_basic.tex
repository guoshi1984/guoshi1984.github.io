...
A B = C\\
Then each column of C can be viewed as a linear combination of columns of A with Bбпs column as coefficients.
1st col of C \\
= $b_11$* 1st Col of A + $b_{21}$* 2nd Col of A + бн + $b_{n1}$* nth Col of A\\
= 
\begin{align*}
( C_1  |  C_2  |  ... | C_n ) \\
= ( A_1  |  A_2  |  ... | A_n )\\
...
Then each row of C can be viewed as a linear combination of rows of B with Aбпs row as coefficients.
1st row of C \\
= $a_11$* 1st Row of B + $a_{12}$* 2nd Row of B + бн + $a_{1n}$* nth Row of B\\
= \begin{align*}
\left( \begin{array}{c}
C_1\\
C_2\\
...
1)	Determinant\\
\begin{align*}
	|A| = \sum_{ \sigma(\pi_j)} a_{1 j_1} a_{2 j_2}бн.a_{n j_n}
\end{align*}
2)	Minors:\\
...
\begin{align*}
	|A+D| = |A| + \sum_{i_1,i_2бн.i_n-1} |A_{i_1, ,i_2бн.i_n-1}| d
+бн.+
...
\end{align*}
\begin{align*}
	|A+D - \lambda I| & = n (n - 1 - \lambda)^{(n-1)} \\
                                     + (n - 1 - \lambda)^n \\
                      &               = (n - 1 - \lambda)*(2n - 1 -\lambda) \\
\end{align*}
