...
\begin{align*}
   \nabla f(x_1, x_2) = (\frac{\partial }{\partial x_1} f(x_1,x_2) \hat x, 
	\frac{\partial }{\partial x_2} f(x_1, x_2) \hat x_2) = (4x_1 \hat x_1, \hat x_2)\\
\end{align*}
\noindent 2) Suppose we start at point $(x^{(0)}, y^{(0)}) = (-\frac{1}{2}, 1)$. Based on the principle of gradient descent, we move point 0 to point 1 along the opposite direction of the gradient. Namely, the direction we move should be
\begin{align*}
	{\bf \hat d} = \frac{1}{||d||} (-4x \hat x_1, -\hat x_2)\\ 
\end{align*}
At point $(x_1^{(0)}, x_2^{(0)})$, $\hat d$ is
\begin{align*}
	{\bf \hat d}^{(0)} = \frac{1}{||d^{(0)}||} (-4x_1^{(0)} \hat x_1, -x_2^{(0)}\hat x_2)\\ 
	 = \frac{1}{||d^{(0)}||} (-4(-\frac{1}{2}) \hat x_1, -1 \hat x_2) 
	 = (\frac{2}{\sqrt{5}}, -\frac{1}{\sqrt{5}})
\end{align*}
3)After finding the direction, we need to determine the step $\alpha$ that we need to move, our next point is
\begin{align*}
...
\end{align*}
4)Error term.\\
Since we can easily know the the minimum point of $f(x_1, x_2)$ is (0,0) without doing any calculation. We can calculate the error term which gives us how far we are still off the minimum point. We define the error term as
\begin{align*}
...
\begin{align*}
	...
	=0
\end{align*}
	Orthogonal!!! This means the error term and moving direction are A- orthogonal. This is a very interesting point. But why?\\
{\bf b. Proof of orthogonality}\\
...
\begin{align*}
	d^{(0)}^T A e^{(1)} = 0
\end{align*}
So by choosing $\alpha$ that minimize $f({\bf x^{(1)})$, we automatically guarantee that the moving direction is A-orthogonal to the error term. The orthogonality relationship is due to the nature that $f({\bf x})$is quadratic. If we take a f(x) with a different form, it is not guaranteed to have this relationship. Proof completed.\\
7) 
...
\begin{align*}
	\alpha & = \frac{d^{(0)}^T A e^{(0)} }{d^{(0)}^T A d^{(0)} }\\
	       & = \frac{d^{(0)}^T A {\bf x_{min} - x^{(0)}} }{d^{(0)}^T A d^{(0)} }\\
\end{align*}
